...
import {ZenConfigFileInfo} from './zen-config'
import {
...
type package_json_zendependency = Pick<
  ...
  'import' | 'traverse_imports' | 'version' | 'workspace'
>
/**
...
export type DependecyScope = 'dependencies' | 'devDependencies' | 'optionalDependencies' | 'peerDependencies'

export type zen_package_tree_dependency = package_json_zendependency & {_depscope: DependecyScope; name: string}
type zen_package_tree = (zen_package_tree_dependency & {
  pack_signature: string
  publish_resolve: string
  version_resolve: string
  /**
   * If this package was the result of another package that has "traverse_imports" to true, then this will be true
   */
 ...
 * @param zen_config The config file info read from ReadProjectsZenConfigFilePath
 *
 * @returns {zen_package_tree} the package tree.
 */

export function GenerateZenPackagesTree(
  rootPackages: zen_package_tree_dependency[],
  zen_config?: ZenConfigFileInfo,
): zen_package_tree {
    ...
    packages.forEach((Package) => {
      ...
      if (fs.existsSync(packagePublishInfoLockFilePackageJsonPath)) {
        ...
        for (const lockPkgName in packagePublishInfoLockFile.pkgs) {
          ...
          let imports_traversed: boolean | undefined = Package.traverse_imports
          if (Package.import === true && zen_config?.Config['hoist-imports'] === true) {
            imports_traversed = true // once hoist-imports is true, imports most be traversed
          }

          _totraverse.push({
            _depscope: Package._depscope,
            import: imports_traversed, // import will be what traverse_imports is for root package
            name: lockPkgName,
            traverse_imports: imports_traversed ?? lockPkg.traverse_imports, // traverse_imports will be either the Parent's package traverse_imports or our items specified traverse import.
 ...
 * @param ignore_workspacePathResolves Will not resolve "workspace:*" if it is workspace package - Used by Publish command to stage publishing a package to zen store.
 * @param force_update Will make "version" be "force_update"
 * @param zen_config The config file info read from ReadProjectsZenConfigFilePath
 *
 ...
 */
export async function ResolveZenPackagesTree(
  ...
  [
    {name: string; resolvedPackagePath: string; resolvedPackagePathIncludesProtocol?: boolean}[],
    {name: string; version_resolve: string}[],
  ]
> {
  const treeResults: {name: string; resolvedPackagePath: string; resolvedPackagePathIncludesProtocol?: boolean}[] = []
  const removedItems: {name: string; version_resolve: string}[] = []
  const LOCKFILEPATH = path.join(cwd, ZENLOCKFILENAME)
  let zenRootDirectory: string

  if (zen_config && zen_config.Config['hoist-imports'] === true) {
    // hoist-imports will add all imports found within this config scope to that root directory.
    // e.g. @mekstuff/pkg1/.zen @mekstuff/pkg2/.zen will instead point to root/.zen (Only 1 .zen folder will exist in that root directory)
    // view it like a workspace setup
    zenRootDirectory = path.join(path.relative(cwd, path.join(zen_config.ConfigRootPath)), '.zen')
  } else {
    zenRootDirectory = path.relative(cwd, '.zen')
  }
  ...
  const HoistImportsFileDataPath = path.join(zenRootDirectory, '.zen-hoist')
  const HoistImportsFileData: {
    hoisted: Record<string, string[]>
  } = fs.existsSync(HoistImportsFileDataPath)
    ? JSON.parse(fs.readFileSync(HoistImportsFileDataPath, 'utf-8'))
    : {
        hoisted: {},
      }

  Tree.forEach(async (item) => {
    // Storing hoisted packages inside a `.zen/.zen-hoist` so we know what packages can be removed from .zen directory when a package is removed
    if (zen_config?.Config['hoist-imports'] === true && item.import === true) {
      const HoistedName = toZenDirectoryImportPackageName(item.name, item.version_resolve)
      const HoistedId = path.relative(zen_config.ConfigRootPath, process.cwd())
      HoistImportsFileData.hoisted[HoistedName] = HoistImportsFileData.hoisted[HoistedName] ?? []
      if (HoistImportsFileData.hoisted[HoistedName].indexOf(HoistedId) === -1) {
        HoistImportsFileData.hoisted[HoistedName].push(HoistedId)
      }
    }
    ...
    if (!item.was_traversed) {
      ...
      if (OLD_ITEM_DATA_FROM_LOCK.workspace !== item.workspace) {
        RequiresPackageManagerInstall.push(item)
      }
      ...
      treeResults.push({
        name: item.name,
        resolvedPackagePath:
          item.workspace && ignore_workspacePathResolves !== true
            ? `workspace:*`
            : item.import
            ? path.join(zenRootDirectory, toZenDirectoryImportPackageName(item.name, item.version_resolve))
            : item.publish_resolve,
        resolvedPackagePathIncludesProtocol: (item.workspace && ignore_workspacePathResolves !== true) === true,
      })
    } else {
  ...
  */
  ...
  if (fs.existsSync(zenRootDirectory)) {
    ...
    dir.forEach((f) => {
      if (f.match(/^\./)) {
        // anything starting with "." within `.zen` is ignored.
        ignored_total += 1
        return
      }
      ...
      if (f.match(/^@/)) {
        ...
        for (const x of dir) {
          ...
          if (zen_config?.Config['hoist-imports'] === true) {
            const HoistedId = path.relative(zen_config.ConfigRootPath, process.cwd())
            const HoistedContent = HoistImportsFileData.hoisted[n]?.filter((f) => f !== HoistedId)
            if (HoistedContent?.length > 0) {
              HoistImportsFileData.hoisted[n] = HoistedContent
            } else {
              delete HoistImportsFileData.hoisted[n]
            }
            if (HoistImportsFileData.hoisted[n] === undefined) {
        ...
        const IsLowLevelInTree = Tree.find(
          (x) => toZenDirectoryImportPackageName(x.name, x.version_resolve) === f && x.import === true,
        )
          ? true
          : false

        if (zen_config?.Config['hoist-imports'] === true && IsLowLevelInTree === false) {
          const HoistedId = path.relative(zen_config.ConfigRootPath, process.cwd())
          const HoistedContent = HoistImportsFileData.hoisted[f]?.filter((f) => f !== HoistedId)
          if (HoistedContent?.length > 0) {
            HoistImportsFileData.hoisted[f] = HoistedContent
          } else {
            delete HoistImportsFileData.hoisted[f]
          }
          if (HoistImportsFileData.hoisted[f] === undefined) {
  ...
  const HasHoistedContent = Object.keys(HoistImportsFileData.hoisted).length > 0
  if (zen_config?.Config['hoist-imports'] === true && HasHoistedContent === true) {
    fs.mkdirSync(path.join(HoistImportsFileDataPath, '..'), {recursive: true})
    fs.writeFileSync(HoistImportsFileDataPath, JSON.stringify(HoistImportsFileData), 'utf-8')
  } else {
    fs.rmSync(HoistImportsFileDataPath, {force: true})
  }
  ...
  ;[...RequiresPackageInjection, ...RequiresPackageManagerInstall].forEach((target) => {
    if (target.import) {
      ...
      if (target.traverse_imports || target.was_traversed || zen_config?.Config['hoist-imports'] === true) {
